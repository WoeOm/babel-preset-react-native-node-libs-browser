# babel-preset-react-native-node-libs-browser
...
Install `babel-preset-react-native-node-libs-browser` in your app:
```sh
npm i babel-preset-react-native-node-libs-browser --save-dev
```
...
```
{
  "presets": ["react-native-node-libs-browser"]
}
```
